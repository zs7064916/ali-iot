...
static int iotx_mc_init(iotx_mc_client_t *pClient, iotx_mqtt_param_t *pInitParams)
{
    ...
    memcpy(connectdata.will.struct_id, pInitParams->will.struct_id, 4);
    connectdata.will.struct_version = pInitParams->will.struct_version;
    connectdata.will.topicName = pInitParams->will.topicName;
    connectdata.will.message = pInitParams->will.message;
    connectdata.will.qos = pInitParams->will.qos;
    connectdata.will.retained = pInitParams->will.retained;
    ...
    pClient->recv_ack_max_time_ms = pInitParams->recv_ack_max_time_ms;
    pClient->recv_ack_max_num = pInitParams->recv_ack_max_num;
...
static int iotx_mc_cycle(iotx_mc_client_t *c, iotx_time_t *timer)
{
    ...
    if (c->recv_ack_max_num < c->keepalive_probes) {
...
static int iotx_mc_keepalive_sub(iotx_mc_client_t *pClient)
{
    ...
    mqtt_info("heartbeat countdown %dms\r\n", iotx_time_left(&pClient->next_ping_time));
    ...
    // utils_time_countdown_ms(&pClient->next_ping_time, pClient->connect_data.keepAliveInterval * 1000);
    utils_time_countdown_ms(&pClient->next_ping_time, pClient->recv_ack_max_time_ms);
