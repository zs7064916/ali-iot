...
#else
    #define mqtt_emerg(...)             do{HAL_Printf(__VA_ARGS__);}while(0)
    #define mqtt_crit(...)              do{HAL_Printf(__VA_ARGS__);}while(0)
    #define mqtt_err(...)               do{HAL_Printf(__VA_ARGS__);}while(0)
    #define mqtt_warning(...)           do{HAL_Printf(__VA_ARGS__);}while(0)
    #define mqtt_info(...)              do{HAL_Printf(__VA_ARGS__);}while(0)
    #define mqtt_debug(...)             do{HAL_Printf(__VA_ARGS__);}while(0)
#endif
...
void *IOT_MQTT_Construct(iotx_mqtt_param_t *pInitParams)
{
    ...
    mqtt_params.recv_ack_max_time_ms  = CONFIG_MQTT_RECV_ACK_MAX_TIME_MS;
    mqtt_params.recv_ack_max_num      = CONFIG_MQTT_RECV_ACK_MAX_NUM;
    ...
    if (pInitParams != NULL) {
        ...
        if (pInitParams->recv_ack_max_time_ms > CONFIG_MQTT_RECV_ACK_MAX_TIME_MS) {
            mqtt_params.recv_ack_max_time_ms = pInitParams->recv_ack_max_time_ms;
        }

        if (pInitParams->recv_ack_max_num > CONFIG_MQTT_RECV_ACK_MAX_NUM) {
            mqtt_params.recv_ack_max_num = pInitParams->recv_ack_max_num;
        }
    } else {
        mqtt_warning("Using default port: [%d]", g_default_sign.port);
        mqtt_params.port = g_default_sign.port;
