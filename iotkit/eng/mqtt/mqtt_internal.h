...
#else
    #define mqtt_emerg(...)             do{feitian_log(LOG_DEBUG, ##__VA_ARGS__);}while(0)
    #define mqtt_crit(...)              do{feitian_log(LOG_DEBUG, ##__VA_ARGS__);}while(0)
    #define mqtt_err(...)               do{feitian_log(LOG_DEBUG, ##__VA_ARGS__);}while(0)
    #define mqtt_warning(...)           do{feitian_log(LOG_DEBUG, ##__VA_ARGS__);}while(0)
    #define mqtt_info(...)              do{feitian_log(LOG_DEBUG, ##__VA_ARGS__);}while(0)
    #define mqtt_debug(...)             do{feitian_log(LOG_DEBUG, ##__VA_ARGS__);}while(0)
#endif
